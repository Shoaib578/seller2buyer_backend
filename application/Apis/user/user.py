...
def GetMySubscription():
    user_id =request.args.get('user_id')
    subscription = text("select  DATEDIFF(day,expiration_date,NOW()) AS remaing_time, expiration_date,start_date,user_subscription_id   from  user_subscription  where user_id="+str(user_id)+" AND NOW() <= DATE(expiration_date) ")
    engine = db.engine.execute(subscription)
